...
def odm_properties(root):
  ...
  # root.set('{http://www.cdisc.org/ns/odm/v1.3}xlink',"http://www.w3.org/1999/xlink")
  # root.set('{http://www.cdisc.org/ns/odm/v1.3}def',"http://www.cdisc.org/ns/def/v2.1")
  ...
  root.set('{http://www.cdisc.org/ns/def/v2.1}Context',"Other")
...
# ISSUE: Hardcoded
def standards():
  standards = ET.Element('{http://www.cdisc.org/ns/def/v2.1}Standards')

  standard1 = ET.Element('{http://www.cdisc.org/ns/def/v2.1}Standard')
...
def metadata_version(oid = 'tbd', name = 'tbd', description = 'tbd'):
  ...
  metadata.set("{http://www.cdisc.org/ns/def/v2.1}DefineVersion", "2.1.7")
  return metadata
...
def translated_text(language, text):
    ...
    translated_text.set('{http://www.cdisc.org/ns/odm/v1.3}lang',language)
...
def origin(type, source):
    origin = ET.Element('{http://www.cdisc.org/ns/def/v2.1}Origin')
    origin.set('Type', type)
    origin.set('Source', source)
    return origin

def leaf(id, href, text_str):
    leaf = ET.Element('{http://www.cdisc.org/ns/def/v2.1}leaf')
    leaf.set("ID", id)
    leaf.set("{http://www.w3.org/1999/xlink}href", href)
    title = ET.SubElement(leaf, '{http://www.cdisc.org/ns/def/v2.1}title')
    title.text = text_str
    return leaf
...
def item_group_defs(domains):
    ...
    for d in domains:
        ...
        igd.set('{http://www.cdisc.org/ns/def/v2.1}Structure', 'tbc')
        igd.set('Purpose', 'Tabulation')
        igd.set('{http://www.cdisc.org/ns/def/v2.1}StandardOID', 'STD.1')
        igd.set('{http://www.cdisc.org/ns/def/v2.1}ArchiveLocationID', 'tbc')
        ...
        ET.SubElement(igd,'{http://www.cdisc.org/ns/def/v2.1}Class', {'Name': goc})
        # ET.SubElement(igd,'{http://www.cdisc.org/ns/def/v2.1}Class').text = goc
        # goc_e.text = goc
    ...
    for d in domains:
        ...
        for v in d['variables']:
          ...
          if vlms:
            ...
            vld = ET.Element('{http://www.cdisc.org/ns/def/v2.1}ValueListDef')
            ...
            for vlm in vlms:
              ...
              wcd = ET.Element("{http://www.cdisc.org/ns/def/v2.1}WhereClauseRef")
              wcd.set('WhereClauseOID', where_clause_oid(d['name'], v['name'], v['testcd'])) 
              item_ref.append(wcd)
              # TODO: WhereClauseRef
              # item_ref.set('{http://www.cdisc.org/ns/def/v2.1}WhereClauseRef'], {)
              #   "{http://www.cdisc.org/ns/def/v2.1}WhereClauseRef":
              #         {
              #             "@WhereClauseOID": "FIX"
              #         }
              # }
...
def range_check(decodes,comparator, soft_hard, item_oid):
    ...
    range_check.set('{http://www.cdisc.org/ns/def/v2.1}ItemOID', item_oid)
...
def where_clause_defs(domains):
    ...
    for d in domains:
        ...
        for v in d['vlm']:
          ...
          for vlm in vlms:
            # debug.append(vlm)
            wcd = ET.Element('{http://www.cdisc.org/ns/def/v2.1}WhereClauseDef')
...
def main():
  define = {}
  # root = ET.Element('ODM', nsmap={None: 'http://www.cdisc.org/ns/odm/v1.3'})
  root = ET.Element('ODM')
  odm_properties(root)
  study = _study()
  # Study -------->
